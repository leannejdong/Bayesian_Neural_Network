...
def main():
    ...
    if problem == 1:
        ...
        input_size = 4
        output_size = 2
        x_train = training_data[1:, 0:input_size]
        y_train = training_data[1:, input_size:input_size+output_size]
        x_test = test_data[1:, 0:input_size]
        y_test = test_data[1:, input_size:input_size+output_size]


    if problem == 2:
        training_data = np.loadtxt("Datasets/4bit.csv", delimiter=',')  # 4-bit parity problem
        test_data = np.loadtxt("Datasets/4bit.csv", delimiter=',')  #
        hidden_size = 4
        input_size = 4
        output_size = 1
